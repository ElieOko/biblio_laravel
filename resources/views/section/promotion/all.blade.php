<x-app-layout>
  <x-slot name="header">
      <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
          {{ __('Liste de toutes les promotions') }}
      </h2>
  </x-slot>
  <div class="flex-none w-full max-w-full px-3 mt-6">
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="p-4 pb-0 mb-0 bg-white rounded-t-2xl">
          ...
          <p class="leading-normal text-sm">Effectif {{$promotions->count()}}</p>
        </div>
            ...
            <div class="w-full max-w-full px-3 mb-6 md:w-6/12 md:flex-none xl:mb-4 xl:w-3/12">
              <div class="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none rounded-2xl bg-clip-border">
                <div class="flex-auto px-1 pt-6">
                  <p class="relative z-10 mb-2 leading-normal text-transparent bg-gradient-to-tl from-gray-900 to-slate-800 text-sm bg-clip-text">{{"@"."Promotion"}} <span class="text-gray-800">{{$promotion->nom}}</span></p>
                  <p class="relative z-10 mb-2 leading-normal text-transparent bg-gradient-to-tl from-gray-900 to-slate-800 text-sm bg-clip-text">{{"@"."Section"}} {{$promotion->section->nom}}</p>
                  @if ($promotion->isTerminal)
                  <p class="relative z-10 mb-2 leading-normal text-transparent bg-gradient-to-tl from-gray-900 to-slate-800 text-red-500 bg-clip-text">#Terminal</p>   
                  @endif
                  <p class="mb-6 leading-normal text-sm">Nombre d'étudiants dans la promotion {{count($promotion->etudiant)}}.</p>
              ...
              <div class="relative flex flex-col h-full min-w-0 break-words bg-transparent border border-solid shadow-none rounded-2xl border-slate-100 bg-clip-border">
                <div class="flex flex-col justify-center flex-auto p-6 text-center">
                  <a href="/promotion/create">
                    ...
                    <h5 class="text-slate-400">Ajouter une Promotions</h5>
                  </a>
                </div>
              </div>
            </div>
